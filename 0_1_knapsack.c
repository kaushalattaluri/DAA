...
void heapify2(priority_queue* p){
	int i;
	for(i=p->queue_size/2-1;i>=0;i--) heapify(p,i);
}

//To insert an element into the priority_queue
void insert(priority_queue* p,Node ele){
	p->array[p->queue_size]=ele;
	p->queue_size++;
	heapify2(p);
}
...
//below two functions implement inbuilt quick sort
//one can also do this using default bubble sort technique
int comparator(const void* a,const void* b){
	Item *x=(Item* )a;
	Item *y=(Item* )b;
	return (double)y->profit/y->weight - (double)x->profit/x->weight;
}
...
void knapsack(Item* arr){
	...
	for(i=0;i<size;i++) printf("weight = %d profit =%d\n",arr[i].weight,arr[i].profit);
	*/
	...
	while(p->queue_size != 0){
		...
		if(current.cw+arr[current.level].weight <= knapsack_weight){
		  ...
		  printf("current level= %d current lower bound=%d current.upper_bound=%d cp=%d cw=%d\n",current.level,current.lower_bound,current.upper_bound,current.cp,current.cw);
	...
	printf("Maximum profit is %d",-global_lb);
